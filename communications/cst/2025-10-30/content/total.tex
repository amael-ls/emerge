...
The equation \eqref{eq::mu-ratio} gives similar predictions (see \ref{fig::pred-ratio}) to \cite{Vallet2006} (currently in use at the \NFI) or Emerge\sidenote{In addition to digitalise the dataset related to the `protocol Oudin', the project Emerge aimed at developing a unique allometry for all tree species in the French forest. They arrived at the allometry \[ \Vtot = \frac{c^2 h}{4 \pi (1 - \sfrac{1.3}{h})} \left( \alpha + \beta \frac{\sqrt{c}}{h} + \gamma \frac{h}{c} \right)\]} \parencite[allometry not used officially by the French \NFI]{Deleuze2014}.
