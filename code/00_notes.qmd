...
### Parameters
I define the following parameters, that will be used to generate data according to equation \eqref{eq::deleuze-modified}:
```{r}
#### Define parameters
...
```
We aim to recover them later with a statiscal model (the values are stored in @tbl-params).
...
: Parameters value for both functional groups {#tbl-params .borderless .hover}
...
It gives the following results (see @tbl-params for real values):
```{r}
...
Simply adding two priors on $\sigma_0$ and $\sigma_2$ like bellow does not necessarily work (it does in my case because the data are informative enough):
...
This gives the following results (see @tbl-params for real values):
```{r}
#| html-table-processing: none
#| hold: true
#| echo: false
...
Now, we consider the same model, but with a correlation between slenderness and hardiness, and we run the same hierarchical model.
```{r}
#| output: false
set.seed(1969 - 08 - 18) # Woodstock seed
...
fit = hierarchical_model$sample(data = stanData, chains = n_chains, parallel_chains = ifelse(n_chains < 4, n_chains, 4),
	refresh = 200, iter_warmup = iter_warmup, iter_sampling = iter_sampling, adapt_delta = 0.95)
```

With a correlation of `{r} round(fake_dt[, cor(fake_hdn, fake_slenderness_cor)], 2)` (-0.51 in the real data), we get the following results (see @tbl-params for real values):
```{r}
#| html-table-processing: none
#| hold: true
#| echo: false
...
```{r}
#| label: fig-posterior_model2-beta0_cor
#| fig-cap: "Trace plot and posterior of $\\beta_0$ for the fifth species"
#| fig-subcap: 
#|   - "Trace plot"
#|   - "Posterior"
...
```{r}
#| label: fig-posterior_model2-sigma_beta0_cor
#| fig-cap: "Trace plot and posterior of $\\sigma_0$"
#| fig-subcap: 
#|   - "Trace plot"
#|   - "Posterior"
...
Note that I made the test with $\sigma = 35$ (instead of $\sigma = `{r} sigma`$). In other words, I provided less informative data. It was  still working, although the parameters' variance increased a lot (makes sense!). I do not show these results.
