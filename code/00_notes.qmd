...
### Complete model, where nothing is provided

::: {.callout-important}
## Quick and dirty way (which does not work)

Simply adding two priors on $\sigma_0$ and $\sigma_2$ like bellow will not work:
...
It seems that this renders the model unidentifiable. Therefore, I am going to try a [non-centred parametrisation](https://betanalpha.github.io/assets/case_studies/hierarchical_modeling.html). Another example is [available here](https://betanalpha.github.io/assets/case_studies/factor_modeling.html#3_Overlapping_Multifactor_Models).

:::

```{stan output.var = "full_model", cache = TRUE}
data {
	// Dimensions and indices
	int N; // Number of individuals
	int S; // Number of species
...
parameters {
	...
	// Variance
	real<lower = 0> sigma; // sd residuals
	real<lower = 0> sigma_beta0; // sd random effect on intercept
	real<lower = 0> sigma_beta2; // sd random effect on slenderness slope
}
