...
if (!file.exists("./bayesian_vallet/pred_dt-7species.rds"))
{
	...
	training_dt = readRDS("./bayesian_vallet/pred_dt-7species.rds")
}
```
...
for (sp in ls_species)
{
	...
	points(stanData_gen$total_volume_m3, pred_yang, col = "#FAB255", pch = 19, cex = 0.45)
	ind = pred_vallet < stanData_gen$bole_volume_m3
	if (any(ind))
		points(stanData_gen$total_volume_m3[ind], pred_vallet[ind], col = "#0F7BA2",
			pch = 19, cex = 0.75)
	legend("topleft", legend = c("Vallet", "Yang", "Pb Vallet"), fill = c("#000000", "#FAB255", "#0F7BA2"))
	axis(1)
	axis(2, las = 1)
	abline(a = 0, b = 1, lwd = 2, col = "#CD212A")

	pgfplots_dt = data.table(obs = stanData_gen$total_volume_m3, vallet = pred_vallet, yang = pred_yang)
	pgfplots_dt[, pb_vallet := as.integer(ind)] # 0 = no problem, 1 = tot < bole
	pgf_file = paste0("../../../writing/article_allometry/data/vallet-yang_", sp_filename, ".csv")
...
if (!file.exists("./bayesian_vallet/comparison_vallet-yang.rds"))
{
	...
	saveRDS(R2D2, "./bayesian_vallet/rsquared_vallet.rds")
	fwrite(comp_dt, "../../../writing/article_allometry/data/comp_dt_vallet.csv")
	fwrite(R2D2, "../../../writing/article_allometry/data/rsquared_vallet.csv")
} else {
	comp_dt = readRDS("./bayesian_vallet/comparison_vallet-yang.rds")
	R2D2 = readRDS("./bayesian_vallet/rsquared_vallet.rds")
}
```

```{r}
#| label: tbl-comp
#| tbl-cap: Comparison of Vallet vs Yang. Highligts in blue when Vallet is rated as best.
...
comp_dt |>
	...
	cols_label(
		...
		best = "Best model",
		elpd_diff = md("$\\Delta$ELPD"),
		se_diff = md("Error $\\Delta$ELPD"),
		wrn_yang = "Warning Yang",
		wrn_vallet = "Warning Vallet"
	) |>
	tab_style(
	...
	tab_style(
		style = cell_fill(color = "#80BCD8AA"),
		locations = cells_body(row = best == "vallet")
	)
```