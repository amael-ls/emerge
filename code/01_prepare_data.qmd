...
inra_arbres[, total_volume_m3 := (v_tronc_verif +
	...
	ifelse(is.na(v_menu_verif), 0, v_menu_verif))/1e3]
...
inra_arbres = unique(inra_arbres[, .(nom_fichier, id_codesessences, circumference_m, h_tot,
	total_volume_m3, v_tronc_verif, v_courrone, genre, id_codesforets, year, presence_NA)])
...
## Keep only column of interest and rename them
inra_arbres_profils = unique(inra_arbres_profils[, .(nom_fichier, arbre, haut, v_bill, circonf_sur_ecorce)])
inra_arbres_profils = inra_arbres_profils[!is.na(v_bill)]
...
setnames(inra_arbres_profils, new = c("unique_id", "tree_id", "at_height", "log_volume_m3", "circumference_m"))
...
There are some `NA` values for the `year` and `ddec` (taper diameter, taken near the taper height `hdec`) variables. I decided to keep these measurement instead of doing `na.omit`, especially for the year variable as it is mostly a useless info especially in ancient method (see @nte-AM).
...
setnames(nfi_data, new = c("species_code_nfi", "plot_id", "tree_id", "circumference_m", "height",
	...
	"cycle", "year", "bole_volume_m3", "genus", "speciesName_sci", "fct_type"))
...
temp = rbind(forest_dt[, .(plot_id, tree_id, genus, circumference_m,
		bole_volume_m3, crown_volume_m3, origin_data)],
