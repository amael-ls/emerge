...
ls_inr = inra_arbres[, nom_fichier] #! IT FAILS TO RUN, AS nom_fichier IS NOT IN inra_arbres ANYMORE
...
Data similar to EMERGE has also been collected in Switzerland, spanning from 1888 and 1974 [@Didion2024]. It represents more than \num{40000} felled trees, with diameter measurements every two meters (when possible) up to the taper height of \qty{7}{\centi\metre}. All the trees have a diameter measured at \qty{1}{\metre}, \qty{3}{\metre}, \qty{5}{\metre}, and so on. The original data provided volumes, however the authors of @Didion2024 never succeded to reconstruct them fully (*personal comm.*). Therefore, they decided to let the reconstruction of the volume open. I reconstruct the volume using the package `TapeR` [@Kublin2013].

But first, I complete the species list, in order to have uniformed names.
```{r}
...
swiss_dt[, length(unique(tree_id))] # 17570 individuals
...
diam007 = diam007[L_coarsestem > 0] # Otherwise I put 7 cm at height 0!
...
n_indiv = unique(swiss_dt[, .(tree_id, sp_code)])
n_indiv = n_indiv[, .N, by = sp_code][order(-N)]

## Compute bole volume using TapeR package (https://link.springer.com/article/10.1007/s10342-013-0715-0)
# Fit taper model to estimate tree profile
if (!dir.exists("./swiss_taper"))
	dir.create("./swiss_taper")

for (sp in n_indiv[, sp_code])
{
	print(paste("Species", sp, "has", n_indiv[sp_code == sp, N], "individuals."))
	filename = paste0("./swiss_taper/", sp, ".rds")

	if (!file.exists(filename))
	{
		if (n_indiv[sp_code == sp, N] < 25)
		{
			warning(paste("Species", sp, "has less than 40 individuals; skipping!"))
			next
		}
		taper_model = TapeR_FIT_LME.f(Id = swiss_dt[sp_code == sp, tree_id],
			x = swiss_dt[sp_code == sp, diameter_height_m/height], # Relative height
			y = swiss_dt[sp_code == sp, 100*diameter], # In cm in the function
			knt_x = c(0.0, 0.1, 0.75, 1.0), ord_x = 4,
			knt_z = c(0.0, 0.1, 1.0), ord_z = 4, IdKOVb = "pdSymm")

		saveRDS(taper_model, paste0("./swiss_taper/", sp, ".rds"))
	}
	
	taper_model = readRDS(filename)
	
	## Compute bole volume from base to cut 7 cm
	# Estimate height at which there is the diameter 7cm
	swiss_dt[sp_code == sp, h7 := E_HDx_HmDm_HT.f(
		Dx = 7,
		Hm = diameter_height_m,
		Dm = 100*diameter, # Must be in cm
		mHt = unique(height),
		sHt = 0,
		par.lme = taper_model$par.lme),
	by = tree_id] # Equals -Inf when there is no solution, i.e., trees are really too small

	# Then compute volume between base to taper height 7 cm
	swiss_dt[sp_code == sp & !is.infinite(h7), bole_volume_m3 := E_VOL_AB_HmDm_HT.f(
		Hm = diameter_height_m,
		Dm = 100*diameter, # Must be in cm
		mHt = unique(height),
		sHt = 0,
	...
	## Compute whole bole volume from base to tip (i.e., including the cone section)
	swiss_dt[sp_code == sp, whole_bole_volume_m3 := E_VOL_AB_HmDm_HT.f(
		Hm = diameter_height_m,
		Dm = 100*diameter, # Must be in cm
		mHt = unique(height),
		sHt = 0,
...
# # Check for one tree out of curiosity
# tree = swiss_dt[sp_code == sp][1, tree_id]

# pred_1 = E_DHx_HmDm_HT.f(Hx = seq(0.5, swiss_dt[.(tree)][1, height], by = 0.5),
# 	Hm = swiss_dt[.(tree), diameter_height_m],
# 	Dm = swiss_dt[.(tree), 100*diameter], # Do not forget the cm!
# 	mHt = swiss_dt[.(tree)][1, height], sHt = 0, par.lme = taper_model$par.lme)

# plot(pred_1$Hx, pred_1$DHx, type = "l", xlab = "Height", ylab = "Predicted diameter")
# points(swiss_dt[.(tree), diameter_height_m], swiss_dt[.(tree), 100*diameter], pch = 19, col = "#CD212A")
...
# Total volume
swiss_dt[, total_volume_m3_cyl := bole_volume_cyl_m3 + crown_volume_m3]
swiss_dt[, total_volume_m3 := whole_bole_volume_m3 + V_coarsebranch + V_finebranch]

# qq = unique(swiss_dt[sp_code == sp, .(tree_id, dbh_m, h7, bole_volume_m3,
# 	bole_volume_cyl_m3, whole_bole_volume_m3, total_volume_m3, total_volume_m3_cyl)])

# plot(qq[, total_volume_m3_cyl], qq[, total_volume_m3], pch = 19,
...
# Interpolation of the diameters every metre, based on tree profile models
swiss_hdec[, interp_diam_spline := ????, by = tree_id]
...
# 	crown_volume_m3, whole_bole_volume_m3, bole_volume_m3, total_volume_m3_cyl, total_volume_m3)])
...
I reconstruct the bole volume using Smalian's and Newton's formul\ae @West2015:
