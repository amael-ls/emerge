...
My reconstruction matches the provided branch volumes except for the trees `{r} compare[abs(vol_tot_branch_m3 - v_br_verif) > 1e-5, tree_id]`. Note that I did like them: for the incomplete trees I use a density of \qty{1000}{\kilogram\per\cubic\metre} to convert the wet weight into a volume when branch's length or circumference are missing.
...
swiss_dt[, c("dbh_m", "height", "circumference_m") := .(DBH/1e3, H_total/10, pi*DBH/1e3)]
swiss_dt[, c("dbh_m", "height", "circumference_m") := .(DBH/1e3, H_total/10, pi*DBH/1e3)]
...
setnames(swiss_dt, old = c("SiteID", "Lat", "Long", "InvYear", "TreeID", "TreeSpecies"),
	new = c("plot_id", "latitude", "longitude", "year", "tree_id", "sp_code"))
```