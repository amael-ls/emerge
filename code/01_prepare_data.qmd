...
if (subsample)
{
	warning("Subsample is TRUE, not all the data are saved! Set it to false to use the whole dataset")
	set.seed(1968 - 08 - 18)
	n = round(forest_dt[, .N]/10)
	selected = sort(sample(x = forest_dt[, .N], size = n, replace = FALSE))

	forest_dt = forest_dt[selected]
	forest_dt[, nb_indiv := .N, by = speciesName_sci]
	forest_dt = forest_dt[nb_indiv > 30]
}
```
...
stanData = list(
	N = forest_dt[, .N],
	N_inra = forest_dt[!is.na(crown_volume_m3), .N],
	S = ind_species[, .N],
...
```{r}
add_name = ifelse(subsample, "", "_full")
