...
$$
v_1 = \begin{cases}
	\frac{2.60 c^2}{4 \pi} & \text{if } h_{\text{dec}} > 2.6 \\
...
nfi_data[, c("v1", "v2", "v3", "d26", "dmed", "cmed", "ddec", "hdec", "verif") := NULL]
...
setnames(nfi_data, new = c("species_code_nfi", "plot_id", "tree_id", "circumference_m", "height", "bole_volume_m3",
	"x_lambert", "y_lambert", "year", "bole_volume_reconstruct_m3", "genus", "speciesName_sci", "fct_type"))
```
...
My reconstruction matches the provided branch volumes except for the trees `{r} compare[abs(vol_tot_branch_m3 - v_br_verif) > 1e-5, tree_id]`. Note that I did like Christine Deleuze and her team: for the incomplete trees I use a density of \qty{1000}{\kilogram\per\cubic\metre} to convert the wet weight into a volume when branch's length or circumference are missing.
...
The data are described in @Didion2024. I reconstruct the volume as follow: TO CONTINUE DESCRIPTION
...
writeVector(x = coords_swiss, filename = paste0(mnt_point, "data/coords_switzerland"), filetype = "ESRI Shapefile",
	overwrite = TRUE)
```
