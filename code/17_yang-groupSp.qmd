...
# Remove NAs (bole volumes only, I do not care if circumference or height have NAs)
...
# Save groups
saveRDS(final_groups, "./yang/ls-groups.rds")
saveRDS(tree_dt, paste0(path_data, "data_Vtot-group.rds"))

# Set key to sort by groups and colour (i.e., France vs Switzerland)
setkey(tree_dt, group, colour)

unique(tree_dt[, .(.N, group), by = speciesName_sci])[order(speciesName_sci)]

ls_groups = tree_dt[, unique(group)]
```
...
The overlap is quite good for all the groups.

```{r}
#| fig-cap: Overlap Fr-Ch by group
#| label: fig-overlap
#| lightbox: 
#|   group: species
...
for (gr in ls_groups)
{
	plot(tree_dt[gr, bole_volume_m3], tree_dt[gr, bole_volume_m3/total_volume_m3],
		col = tree_dt[gr, colour], xlab = "Bole volume", ylab = "r", axes = FALSE, pch = 19,
		cex = 0.85)
...
if (!dir.exists(savePath))
	stop("You should run '11_yang.qmd'")
...
for (gr in ls_groups)
{
	...
	## Run model
	if (file.exists(paste0(savePath, gr, ".rds")))
	{
		fit = readRDS(paste0(savePath, gr, ".rds"))
	} else {
		...
		## Save results
		fit$save_output_files(dir = savePath, basename = gr, random = FALSE)
		saveRDS(fit, paste0(savePath, gr, ".rds"))
	}
...
## Run for groups that had mixing problems

I run the simplified model for the groups that failed, either by divergences or by bad mixing with the full model, \ie, where not `success_full`, haha, I am so funny. HILARIOUS!!!
...
for (gr in ls_pb)
{
	...
	## Run model
	if (file.exists(paste0(savePath, gr, "_submodel.rds")))
	{
		fit = readRDS(paste0(savePath, gr, "_submodel.rds"))
	} else {
		fit = model$sample(data = stanData, chains = n_chains, parallel_chains = min(n_chains, 4),
			iter_warmup = 1500, iter_sampling = 1000, max_treedepth = 10)

		## Save results
		fit$save_output_files(dir = savePath, basename = paste0(gr, "_submodel"), random = FALSE)
		saveRDS(fit, paste0(savePath, gr, "_submodel.rds"))
	}
...
if (!file.exists("./yang/group-success.rds"))
	saveRDS(success_dt, "./yang/group-success.rds")
```
