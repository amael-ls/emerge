...
Note that whatever I try to model, there will always be intractibility somewhere! Either I model both components, trunk and crown, with a multivariate lognormal, and the total volume is untractable (sum of two lognormals), or I model the trunk volume, $\Vtot$ and the ratio, $r$, and $\Vbft$ is untractable. Indeed:
