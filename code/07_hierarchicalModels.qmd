...
## Joined model with components

I try here a first joined model on the taper volume, $\Vbft$, and the crown volume, $\Vcro$, which gathers small branches, large branches, and forks. I assume a lognormal distribution for both volumes, so I will transform both with a $\log$ and then use a multivariate normal distribution. As a simple test, I use an intercept only, no explanatory variables.

::: {.callout-important}

## How to retrieve the total volume?

The total volume, $\Vtot$, is the sum of the trunk and crown:
$$
\Vtot = \Vbft + \Vcro.
$$
...
p = inra$bole_volume_m3/inra$total_volume_m3

plot(log(inra$total_volume_m3), log(p/(1 - p)))
