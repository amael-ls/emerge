...
```{stan output.var = "97_eq15", cache = TRUE}
data {
	// Dimensions
...
#### First run, heteroskedasticity ignored
```{r}
## Run stan model
# Data
stanData = list(
...
# Running
results = 97_eq15$sample(data = stanData, chains = n_chains, parallel_chains = n_chains)
...
As expected, heteroskedasticity is clearly visible on the residuals, with an increase of variance with diameter and height (combined here as a 'cylinder volume'---although cm mixed with metres).
```{r}
plotResiduals(sim_dharma, data[, dbh^2*height], quantreg = TRUE, xlab = "D²h", main = "")
```


#### First run, heteroskedasticity considered
...
## Run stan model
# Compile model
model = cmdstan_model("./97_eq15_heterosked.stan")
...
# Running
results_heterosked = model$sample(data = stanData, chains = n_chains, parallel_chains = n_chains,
...
lazyPosterior(results_heterosked$draws("var_pred[1]"))
lazyPosterior(results_heterosked$draws("var_pred[40]"))
...
plot(data[, dbh^2*height], y, pch = 19, axes = FALSE, xlab = "Diameter", ylab = "Variance")
f = function(x, sigma1, c1)
	return(sigma1*x^c1)
...
As can be seen, the residuals are good when heteroskedasticity is considered
```{r}
plot(sim_dharma, quantreg = TRUE)
plotResiduals(sim_dharma, data[, dbh], quantreg = TRUE)
```
