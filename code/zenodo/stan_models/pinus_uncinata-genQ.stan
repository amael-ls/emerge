...
functions {
	vector r_4_params(vector x, row_vector pars) // Function for the sub-model (4 parameters) with logit
	{
		/*
...
generated quantities {
	...
	vector [N_new] r_gen_mean = r_4_params(bole_volume_m3_new, [alpha, beta_, gamma, delta]); // Average ratio
	vector[N_new] v_gen;
	...
	vector [N_new] shape1_new = phi*r_4_params(bole_volume_m3_new, [alpha, beta_, gamma, delta]);
	vector [N_new] shape2_new = phi*(1 - r_4_params(bole_volume_m3_new, [alpha, beta_, gamma, delta]));

	{
		...
		v_gen_mean = 1/alpha * bole_volume_m3_new .^
			( 1 - (log(r_4_params(bole_volume_m3_new, [alpha, beta_, gamma, delta])) - log(alpha)) ./
