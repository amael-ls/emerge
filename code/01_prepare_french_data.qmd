...
In the old data (INRA), the forest structure is unknown. however, in the data collected in 2009--2010, the forest structure is recorded. I clean it and translate it here.

```{r}
setDT(codesstructures)

codesstructures = unique(codesstructures[, .(structure_uniformise, nomstructure_uniformise)])
setnames(codesstructures, new = c("str_code", "str_name"))
codesstructures[str_name == "inconnu", str_name := "unknown"]
codesstructures[str_name == "un étage", str_name := "single-layered"]
codesstructures[str_name == "jardinée", str_name := "uneven-aged"]
codesstructures[str_name == "deux étages", str_name := "two-layered"]
codesstructures[str_name == "taillis", str_name := "coppice"]
codesstructures[str_name == "TSF", str_name := "coppice-with-standards"]

codesstructures[, str_name := as.character(str_name)]
```
...
Here, I join both profile and branches tables. As said in the previous section, I do no check the volume for the twigs and I use those provided by Emerge.

```{r}
## Join both profile and branches table
build_check_data = function(branches, profiles, trees, structure, species, plots)
{
	...
	ls_names = c("tree", "id_codesforets", "id_codesessences", "id_codesstructures", "c130",
		"v_menu_verif", "hauteurdecoupe7_m")
	...
	emerge = merge.data.table(emerge, trees[, .(tree, id_codesforets, id_codesessences, id_codesstructures,
		taper_height = hauteurdecoupe7_m, circumference_m = c130/100, twig_volume = v_menu_verif)],
		by.x = "tree_id", by.y = "tree")

	emerge = merge.data.table(emerge, structure, by.x = "id_codesstructures", by.y = "str_code", all.x = TRUE)
	emerge[, id_codesstructures := NULL]
...
emerge_2009 = build_check_data(emerge_bure09_branches, emerge_bure09_profils, emerge_bure09_arbres,
	codesstructures, codesessences, codesforets)
