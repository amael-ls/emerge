...
```{r}
test = function (c130, htot, hdec = NULL, dec = 7, tax_ref = NULL, ess = NULL, 
	espar = NULL, nomess = NULL, typevol = "total", conditionel = TRUE, 
	dataframe = FALSE) 
{
	variables = list(c130 = c130, htot = htot, hdec = hdec, 
		dec = dec, tax_ref = tax_ref, ess = ess, espar = espar, 
		nomess = nomess, typevol = typevol, conditionel = conditionel)
	
	X = data.frame(variables[!sapply(variables, is.null)])

	get_tarEmerge = function(c130, htot, hdec = NULL, dec = 7,  tax_ref = NULL, ess = NULL, espar = NULL,
		nomess = NULL, typevol = "total", conditionel = TRUE)
	{
		nobs = length(c130)
		...
		if (!is.null(espar) & is.null(nomess) & is.null(tax_ref) & is.null(ess)) # espar is the NFI code
		{
			if (sum(is.na(espar)) > 0) # Check NAs
				stop("Missing values of 'espar'")

			if (length(espar) < nobs) # Recycling, not very careful here
				espar = rep_len(espar, length.out = nobs)

			liste = unique(inventR::ListTarEmerge[!is.na(inventR::ListTarEmerge$espar), 
				c("espar", "Vtot_a", "Vtot_b_hdn", "Vtot_c_hsurd", 
					"Vtige_d", "Vtige_e_logis", "Vtige_f_hdn", 
					"Vtige_g_invc130", "Vhoup_Alpha", "Vhoup_B")]) # Get coefficients Emerge
			matcoefs = as.matrix(liste[charmatch(espar, liste$espar), 
				c("Vtot_a", "Vtot_b_hdn", "Vtot_c_hsurd", "Vtige_d", 
					"Vtige_e_logis", "Vtige_f_hdn", "Vtige_g_invc130", 
					"Vhoup_Alpha", "Vhoup_B")])
		} else {
			stop("Provide espar!")
		}
		...
		c13 = c130/100
		cDec = dec * pi/100 # Circumference in m from a diameter dec provided in cm
		hdn = sqrt(c13)/htot
		hsurd = htot/(c13)
		corr130 = (1 - 1.3/htot)
		volCyl = (c13)^2 * htot/(4 * pi * corr130^2)
		print(paste("Volume cyl:", round(volCyl, 5)))
		.Xform = matcoefs[, c("Vtot_a", "Vtot_b_hdn", "Vtot_c_hsurd")] * 
			matrix(c(rep(1, nobs), hdn, hsurd), ncol = 3, byrow = FALSE)
		formTot = .rowSums(.Xform, n = ncol(.Xform), m = nrow(.Xform), 
			na.rm = TRUE)
		print(paste("Form tot:", round(formTot, 5)))
		.XFCT = matcoefs[, c("Vtige_d", "Vtige_e_logis", "Vtige_f_hdn", 
			"Vtige_g_invc130")] * matrix(c(rep(1, nobs), logis_hdec, 
			hdn, 1/c13), ncol = 4, byrow = FALSE)
		FCT = .rowSums(.XFCT, n = ncol(.XFCT), m = nrow(.XFCT), 
			na.rm = TRUE)
		print(paste("FCT:", round(FCT, 5)))
		if (any(FCT > 0.95)) {
			if (conditionel) {
				FCT[FCT > 0.95] = 0.8
			}
		...
		beta = matcoefs[, c("Vhoup_B")]
		alpha = matcoefs[, c("Vhoup_Alpha")]
		Cmax = unname((c13/corr130) * ((3 - beta) * (1/FCT - 
			1)/(3 * alpha))^(1/3))
		decHoup = ifelse(Cmax > cDec, (1 - ((cDec/c13 * corr130) * 
			(((3 - beta)/(3 * alpha)) * (1/FCT - 1))^(-1/3))^(3 - 
			beta)), 0)
		volHoup = (formTot - formTige) * volCyl * decHoup
		decTige = ifelse(c13 > cDec, (1 - (cDec * corr130/c13)^3), 
			0)
		if (any(0.75 * c130/pi > dec)) {
			volTige = formTige * volCyl * decTige
		}
		else {
			volTige = formTige * volCyl * decTige
			warning(paste(" This species:", ess, ", Htot =", 
				htot, ", Hdec =", hdec, ", with .75*Diam  =", 
				round(0.75 * c130/pi, 0), "<", dec, "= dec is too large for valuable calculation!"))
		}
		if (typevol == "tige") {
			return(volTige)
		}
			...
			if (typevol == "houp") {
				...
				volTot = volTige + volHoup
				return(volTot)
			}
		}
	} # --- END FUNCTION get_tarEmerge

	res = get_tarEmerge(c130 = c130, htot = htot, hdec = hdec, 
		dec = dec, tax_ref = tax_ref, ess = ess, espar = espar, 
		nomess = nomess, typevol = typevol, conditionel = conditionel)
	if (!dataframe) {
		return(res)
...
library(inventR)
library(stringi)
aa = copy(ListTarEmerge)
setDT(aa)

keepCols = c("espar", "nom_lat", "Vtot_a", "Vtot_b_hdn", "Vtot_c_hsurd",
	"Vtige_d", "Vtige_e_logis", "Vtige_f_hdn", "Vtige_g_invc130",
	"Vhoup_B", "Vhoup_Alpha")

aa = aa[espar %in% c("61", "09", "62", "51", "52", "64", "03"), ..keepCols]
setnames(aa, old = "nom_lat", new = "speciesName_sci")

aa[stri_detect(speciesName_sci, regex = " subsp."),
	speciesName_sci := stri_sub(speciesName_sci, to =
		stri_locate(speciesName_sci, regex = " subsp.", get_length = TRUE)[, "start"] - 1)]
setkey(aa, speciesName_sci)

TarEmerge(250, 20, espar = "09", typevol = "tot")
test(250, 20, espar = "09")

sum(as.numeric(aa["Fagus sylvatica", .(Vtot_a, Vtot_b_hdn, Vtot_c_hsurd)]) * c(1, sqrt(2.50)/20, 20/2.50))*
	(2.50^2*20)/(4*pi*(1 - 1.3/20)^2)
```