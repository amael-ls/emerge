...
```{r}
factor_gohon = function(c, h, hdec, params, species = NULL, speciesCol = "speciesName_sci", isVol = TRUE)
{
	if (!is.null(species))
	{
		if (is.data.table(params) & key(params)[1] == speciesCol)
		{
			if (params[.(species), .N,  nomatch = 0] == 0)
				stop(paste0("Species <", species, "> not found"))
			if (params[.(species), .N] == 1)
			{
				params = as.numeric(params[.(species), .(alpha, beta, gamma, delta, eta)])
				names(params) = c("alpha", "beta", "gamma", "delta", "eta")
			} else {
				stop(paste0(params[.(species), .N], " set of parameters found for species <", species, ">"))
			}
		} else {
			stop(paste0("Species != null: params must be a data table with <", speciesCol, "> col as the first key"))
		}
	}

	formFactor = params["alpha"] + params["beta"]*c + params["gamma"]*sqrt(c)/hdec +
		params["delta"]*sqrt(hdec)/(c^2*h) + params["eta"]*(1 - hdec/h) # Formula checked in F. Gohon's rmd file.

	if (isVol)
		return (c^2*h/(4*pi*(1 - 1.3/h)^2) * formFactor)

	return(formFactor)		
}

params = fread("../data/gohon_3.csv")
setkey(params, sp_code)
dt_interp = forest_dt_full["Quercus petraea"][origin_data == "nfi", .(circumference_m, height, taper_height)]

dt_interp[, pred_vol := factor_gohon(circumference_m, height, taper_height, params,
	species = "02", speciesCol = "sp_code")]
...
### Exploration du modèle de @Zhou2021

@Zhou2021 propose un coefficient d'expansion 'dynamique' entre le volume du tronc et le volume du houppier. Par dynamique, ils entendent un coefficient qui change selon la circonférence et la hauteur par exemple. Une partie de leur modèle est basé sur le facteur de forme de @Vallet2006.
