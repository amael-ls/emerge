...
Le facteur de forme, $\ff$, a été introduit par @Vallet2006, puis a été modifié lors du projet Emerge (@Deleuze2013_b) afin de corriger les problèmes survenant pour les petits arbres (explosion du facteur de forme pour compenser un dbh proche de 0). Ce nouveau facteur de forme est défini par:
$$
...
#| fig-cap: "Figures pour *Quercus robur* avec \\num{5000} sélectionnés"
#| fig-subcap: 
#|   - "Volume BFT *v.s.* circonférence"
#|   - "Coefficient de forme (équation @eq-ff) *v.s.* circonférence"
#| layout-ncol: 2
#| output: false

## Volume vs c
plot(quercus_rob[, circumference_m], quercus_rob[, bole_volume_reconstruct_m3], pch = ".",
	xlab = "$c$", ylab = "$\\Vbft$", axes = FALSE)

axis(1)
axis(2, las = 1)

## Form factor vs c
plot(quercus_rob[, circumference_m], quercus_rob[, formBFT_reconstruct], pch = ".", xlab = "$c$",
	ylab = "$\\ff$", axes = FALSE)
...
```
### Tarifs IFN et Emerge

Les volumes sont calculés avec des formules à trois entrées pour les arbres à mesures complètes, et à une entrée pour les arbres qui n'ont que la circonférence de mesurée. Ensuite, au sein de chaque groupe, il y a différentes formules. L'unique moyen de savoir quelle formule utiliser est de regarder quels coefficients ne sont pas `NA` et quelle fonction est utilisée (voir plus bas pour plus de détails).
...
#### Equations
Je réécris ci-dessous les équations des facteurs de formes avec l'abus de notation suivant:
$$
c = \min(c, 3).
$$ {#eq-c}
