...
# Define UI
user_interface = page_sidebar(
	title = "MCMC pairs",
	sidebar = sidebar(
		helpText("Select two parameters to plot their posteriors and visually check there is no problem"),
		selectInput(
			inputId = "params1",
	...
	card(
		card_header(class = "bg-dark", "Result plot"),
		card_footer("Graphics made with Shiny and Bayesplot packages"),
		plotOutput("mcmc")
	)
)
...
server = function(input, output)
{
	...
	# output$mcmc = renderPlot({
	# 	mcmc_scatter(x = fit$draws(c(paste0(input$params1, "[1]"), paste0(input$params2, "[1]"))),
	# 		alpha = 1, np = nuts_params(fit))
	# })
}
