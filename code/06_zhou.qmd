...
```{r}
## Subset data
forest_dt = forest_dt[(origin_data == "inra") & !(presence_NA)]
...
$$ {#eq-flexible}
However, 4 parameters seems to be impossible to fit (too many degrees of freedom I think) and I set $\alpha = 1$. Indeed, this parameter is involved in the maximum height of the $\mu$, reached for:
$$
x^{*} = \frac{b}{\alpha} + \frac{1}{\beta} \left( 1 - W_p \left[ -\frac{\gamma}{\alpha} e^{\beta b/\alpha + 1} \right] \right),
$$
where $W_p$ is the positive branch of the Lambert function, and is then also involved in the curvature.
...
# forest_dt[, slenderness := height/circumference_m]

for (sp in ls_species)
{
	count = count + 1
	plot_sp(fit, sp, count, stanData, fun = ff, args = c("beta_", "gamma","b"), pred = TRUE, n_subsample = 1)
	# plot_sp(fit, sp, count, stanData, fun = ff, args = c("beta_", "gamma","b"), pred = TRUE, n_subsample = 10,
	# 	selected_variable = "hdn", n_bins = 4, pal = "Egypt")
}
```
...
## Get residuals for Fagus sylvatica
n_sampling = fit$metadata()$iter_sampling
...
dharmaQuant = DHARMa::testQuantiles(simulationOutput = sim_dharma,
	predictor = forest_dt[start:end, rankTransform(bole_volume_m3)], plot = FALSE)

# Add the rank transformed predictor and the prediction + height class for colour
pgfplots = data.table(bole_volume_rt = forest_dt[start:end, rankTransform(bole_volume_m3)],
	residuals = sim_dharma$scaledResiduals)
setorder(pgfplots, bole_volume_rt)
...
fwrite(pgfplots, "./pgfplots/residuals_ratio.csv")

#### Export the ratio data and bole volume + add manually the fitted curve
fwrite(forest_dt[start:end, .(bole_volume_m3, ratio = total_ratio)], "./pgfplots/ratio.csv")

apply(X = fit$draws("beta_[4]"), FUN = mean, MARGIN = 3)
apply(X = fit$draws("b[4]"), FUN = mean, MARGIN = 3)
apply(X = fit$draws("gamma[4]"), FUN = mean, MARGIN = 3)
...
## Difference between @Vallet2006, Emerge, and the newly developed allometries
...
## Compute Vallet 2006 volume 7 species available (listed in sp_dt)
...
## Compute Emerge volume. WATCH OUT: it is necessary to specify dec = 0 to get total volume
for (current_sp in sp_dt[, speciesName_sci])
	forest_dt[current_sp, emerge_predVolume := TarEmerge(c130 = circumference_m*100,
		htot = height, espar = sp_dt[current_sp, emerge], typevol = "total", dec = 0)]
...
fwrite(forest_dt["Fagus sylvatica", .(total_volume_m3, my_predVolume, emerge_predVolume, vallet_predVolume)],
	"fagSyl-vol.csv")

fwrite(forest_dt["Abies alba", .(total_volume_m3, my_predVolume, emerge_predVolume, vallet_predVolume)],
	"abiAlb-vol.csv")
```

```{r}
#| label: fig-obs_vs_me-emerge-vallet
#| fig-cap: Total volume predicted by my model, `inventR::TarEmerge`, and @Vallet2006 *vs* observed
#| fig-subcap:
#|   - "Abies alba"
#|   - "Fagus sylvatica"
#|   - "Picea abies"
...
for (current_sp in sp_dt[, speciesName_sci])
{
	plot(forest_dt[current_sp, total_volume_m3], forest_dt[current_sp, my_predVolume], pch = 19,
		lwd = 0, cex = 0.75, axes = FALSE, xlab = "Observed", ylab = "Predicted", col = "#FAB255",
		ylim = c(0, max(forest_dt[current_sp, .(my_predVolume, emerge_predVolume, vallet_predVolume)])))
	points(forest_dt[current_sp, total_volume_m3], forest_dt[current_sp, emerge_predVolume],
		pch = 19, lwd = 0, cex = 0.6, col = "#0F7BA2")
	points(forest_dt[current_sp, total_volume_m3], forest_dt[current_sp, vallet_predVolume],
		pch = 19, lwd = 0, cex = 0.6, col = "#43B284")
	axis(1)
	axis(2, las = 1)
	abline(a = 0, b = 1, lwd = 2, col = "#CD212A")
	legend(x = "topleft", legend = c("Mine", "TarEmerge", "Vallet"),
		fill = c("#FAB255", "#0F7BA2", "#43B284"), bty = "n")
}
```
...
#| fig-cap: Total volume predicted by my model, `inventR::TarEmerge`, and @Vallet2006 on the French NFI data. The arrows depict the parametrisation range
#| fig-subcap:
#|   - "Abies alba"
#|   - "Abies alba"
#|   - "Fagus sylvatica"
...
for (sp_ind in sp_dt[, sp_index])
{
	...
	data_trees[sp_dt[sp_index == sp_ind, emerge], emerge_predVolume := TarEmerge(c130 = circumference_m*100,
		htot = height, espar = sp_dt[sp_index == sp_ind, emerge], typevol = "total", dec = 0)]
	...
	plot(data_trees[sp_dt[sp_index == sp_ind, emerge], bole_volume_m3],
		...
		pch = 20, lwd = 0, cex = 0.75, axes = FALSE, col = "#43B28444", xlab = "Bole volume", ylab = "Total volume",
		ylim = c(0, max(data_trees[sp_dt[sp_index == sp_ind, emerge],
			.(total_volume_m3, emerge_predVolume, my_predVolume)], na.rm = TRUE)))
	points(data_trees[sp_dt[sp_index == sp_ind, emerge], bole_volume_m3],
		data_trees[sp_dt[sp_index == sp_ind, emerge], emerge_predVolume],
		pch = 20, lwd = 0, cex = 0.6, col = "#0F7BA244")
	points(data_trees[sp_dt[sp_index == sp_ind, emerge], bole_volume_m3],
		data_trees[sp_dt[sp_index == sp_ind, emerge], my_predVolume],
		pch = 20, lwd = 0, cex = 0.6, col = "#FAB25544")
	axis(1)
	axis(2, las 1= 1)
	Arrows(x0 = sp_dt[sp_index == sp_ind, min_v], y0 = 0.1, x1 = sp_dt[sp_index == sp_ind, max_v],
		y1 = 0.1, arr.type = "triangle", code = 3, arr.adj = 1) # A bit useless given pred vs obs!
	abline(a = 0, b = 1, lwd = 2, col = "#CD212A")
...
#### Recreate the study of @Vallet2006 (Bayesian version)
...
for (current_sp in c("Fagus sylvatica", "Quercus petraea")) # The only two species in common
{
	...
	if (!file.exists(filename))
	{
		results = vallet_bayesian$sample(data = stanData, chains = n_chains, parallel_chains = min(n_chains, 4),
			max_treedepth = 12)
		results$save_output_files(dir = "./",
			basename = stri_sub(filename, to = stri_locate(filename, regex = ".rds")[, "start"] - 1),
			random = FALSE)
...
#### Create simulations based on @Vallet2006 (and **not** on the Bayesian version)
It is possible to create simulations on new data based on @Vallet2006, as the following table is provided:

::: {#tbl-table_1 .borderless .hover}

| species                 | $\alpha$ | $\beta$ | $\gamma$ | $\delta$ | residuals |
|------------------------:|:---------|:--------|:---------|:---------|:----------|
| *Abies alba*            | 0.550    | 
| *Fagus sylvatica*       | 0.395    | 
| *Picea abies*           | 0.631    | 
| *Pinus pinaster*        | 0.235    | 
| *Pinus sylvestris*      | 0.297    | 
| *Pseudotsuga menziesii* | 0.534    | 
| *Quercus petraea*       | 0.471    | 

Species-specific parameters with std. dev between parentheses [@Vallet2006]. 
...
It seems that the best model so far, in terms of parameters uncertainty, residuals and PSIS-Leave-One-Out [@Vehtari2017], is the the flexible model in @sec-flexible.

So now it is time to try a multivariate model, with both $\Vbft$ and $\Vtot$ as output variables. Indeed, with the above method, $\Vtot$ completely relies on $\Vbft$, which therefore must be as 'clean' as possible. Therefore it worth trying some models on $\Vbft$. I already started investigating multivariate models on simple examples in `04_test_multivariate.qmd`. As a first step, I will actually start by a model where I fit both volumes, but without correlations in the parameters.
...
## Range of parameterisation {.appendix}
