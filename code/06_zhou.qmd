...
generated quantities {
	array[N_new] real log_lik;
	{
		...
		for (i in 1:N_new)
			log_lik[i] = normal_lpdf(total_volume_m3_new |
			vallet(circumference_cm_new, hdn_new, vec_params, N_params) ./ (40000*pi()) .*
...
for (current_sp in c("Fagus sylvatica", "Quercus petraea")) # The only two species in common
{
	...
	filename = paste0("./loo_beta_", stri_replace(str = current_sp, replacement = "-", regex = " "), ".rds")
	if (!file.exists(filename))
	{
		...
		saveRDS(loo_beta, filename)
	} else {
		loo_beta = readRDS(filename)
	}
	...
	filename = paste0("./loo_vallet_", stri_replace(str = current_sp, replacement = "-", regex = " "), ".rds")
	if (!file.exists(filename))
	{
		loglik_vallet = vallet_loglik$generate_quantities(fitted_params = results$draws(inc_warmup = FALSE),
			data = stanData_new, parallel_chains = min(n_chains, 4))

		loglik_vallet$draws("log_lik")
