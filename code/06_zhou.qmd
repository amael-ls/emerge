...
## Second model: relation between $\Vzh$ and $\Vtot$
...
$$
which is the equation 1 of @Zhou2021, with $\Omega = \ln(p - \exp[-kx])/\ln(x)$. Then, to get the second equation of @Zhou, just (see his appendices) set $p = m/d$, compute many stuff, and you will reach:
$$
y = \frac{x}{m - d\exp[-kx]},
$$
that is to say,
...
# Broadleaves
plot(forest_dt[fct_type == "broadleaf", bole_volume_m3], forest_dt[fct_type == "broadleaf", total_ratio], pch = 20,
...
# Conifers
plot(forest_dt[fct_type == "conifer", bole_volume_m3], forest_dt[fct_type == "conifer", total_ratio], pch = 20,
...
# Broadleaves
plot(forest_dt[fct_type == "broadleaf", circumference_m], forest_dt[fct_type == "broadleaf", vallet_ratio], pch = 20,
...
# Conifers
plot(forest_dt[fct_type == "conifer", circumference_m], forest_dt[fct_type == "conifer", vallet_ratio], pch = 20,
...
```{r}
## Extract average ratio bole/tot volume
simVol = apply(X = fit$draws("v_gen"), MARGIN = 3, FUN = mean)

## Compute average pred volume
forest_dt[, my_predVolume := simVol]

## Compute Vallet 2006 volume, among "chene", "hetre", "douglas", "epicea", "pin.sylvestre", "pin.maritime", "sapin"
forest_dt["Fagus sylvatica", vallet_predVolume := TarVallet(c130 = circumference_m*100, htot = height, ess = "hetre")]

## Plots
# Pred vs obs
plot(forest_dt["Fagus sylvatica", total_volume_m3], forest_dt["Fagus sylvatica", my_predVolume], pch = 20,
	axes = FALSE, xlab = "Observed", ylab = "Predicted", col = "#FAB25544",
	ylim = c(0, max(forest_dt["Fagus sylvatica", .(my_predVolume, vallet_predVolume)])))
points(forest_dt["Fagus sylvatica", total_volume_m3], forest_dt["Fagus sylvatica", vallet_predVolume], pch = 20,
	col = "#0F7BA244")
axis(1)
axis(2, las = 1)
...
# Mine vs Vallet
plot(forest_dt["Fagus sylvatica", vallet_predVolume], forest_dt["Fagus sylvatica", my_predVolume], pch = 20,
...
#! It would worth it to do the extraction for the 7 concerned species here! So far only Fag syl

db = connect_db()

#* ----------------------------- Query trees 2019 - 2023 ----------------------------- #
data_trees = exec_req(conn = db,
	req = "
	SELECT DISTINCT
		tree_dt.NPP AS plot_id, tree_dt.A AS tree_id,
	...
	WHERE
		tree_dt.ESPAR = '09' AND -- Fagus sylvatica
...
data_trees[30092:30097]
data_trees = data_trees[bole_volume_m3 < 20] # This is to remove the tree (plot = 22-76-119-1-061T, tree = 3)
...
stanData_new = list(
	...
	# Predictors
	SB = forest_dt[, bole_volume_m3], # Same as reconstruct one
	bole_volume_m3_new = data_trees[, bole_volume_m3],
...
## Compute average pred volume
data_trees[, my_predVolume := posteriorVol]

## Plot
plot(data_trees[, total_volume_m3], data_trees[, my_predVolume], pch = 20, axes = FALSE,
	xlab = "Vallet", ylab = "Mine")
axis(1)
axis(2, las = 1)
abline(a = 0, b = 1, lwd = 2, col = "#CD212A")
```
