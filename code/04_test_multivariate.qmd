...
fit_bis = model_bis$sample(data = stanData, chains = n_chains, parallel_chains = ifelse(n_chains < 4, n_chains, 4),
	seed = NULL, refresh = 250, max_treedepth = 12, save_warmup = TRUE,
	iter_sampling = iter_sampling, iter_warmup = iter_warmup, adapt_delta = 0.8)
```
...
```{r}
gamma_est = round(apply(X = fit_bis$draws("gamma", inc_warmup = FALSE), MARGIN = 3, FUN = mean), 2)
sigma_act_est = round(apply(X = fit_bis$draws("sigma_diag_actor", inc_warmup = FALSE), MARGIN = 3, FUN = mean), 2)
sigma_blo_est = round(apply(X = fit_bis$draws("sigma_diag_block", inc_warmup = FALSE), MARGIN = 3, FUN = mean), 2)
```
